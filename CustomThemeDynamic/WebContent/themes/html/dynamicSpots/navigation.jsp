...
<c:if test="${primeNavigation && param.primeRoot}">
	<span style="display:none;" id="wpthemePrimeRoot" data-nm-level="0" data-nm-primed="<portal-fmt:out><portal-core:navigationNodePriming navigationNode="${wp.identification[wp.selectionModel.selectionPath[0]]}" metaData="${navHiddenMetadata}" considerChildren="false" includeRoles="true" /></portal-fmt:out>"></span>
</c:if>

<c:forEach var="node" items="${wp.selectionModel.selectionPath}" varStatus="status" begin="${startLevel}" end="${endLevel}" step="1">
	...
	<c:if test="${(selectionPathLength > level) && wp.navigationModel.hasChildren[wp.selectionModel.selectionPath[level]]}">
		<c:forEach var="node" items="${wp.navigationModel.children[wp.selectionModel.selectionPath[level]]}" varStatus="childrenStatus">
